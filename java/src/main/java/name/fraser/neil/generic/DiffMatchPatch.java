...
import name.fraser.neil.generic.IGenericText.T;
...
/**
   ...
   */
  protected static class LinesToCharsResult<char_t> {
    protected Text<Integer> chars1;
    protected Text<Integer> chars2;
    protected List<char_t> lineArray;

    protected LinesToCharsResult(Text<Integer> chars1, Text<Integer> chars2,
        List<char_t> lineArray) {
      this.chars1 = chars1;
      this.chars2 = chars2;
   ...
   * @return Linked List of Diff objects.
   */
  private LinkedList<Diff<char_t>> diff_main(Text<char_t> text1, Text<char_t> text2,
   ...
   * @return Linked List of Diff objects.
   */
  private LinkedList<Diff<char_t>> diff_compute(Text<char_t> text1, Text<char_t> text2,
                                        boolean checklines, long deadline) {
    ...
    if (hm != null) {
      ...
      LinkedList<Diff<char_t>> diffs_a = diff_main(text1_a, text2_a,
                                           checklines, deadline);
      LinkedList<Diff<char_t>> diffs_b = diff_main(text1_b, text2_b,
                                           checklines, deadline);
      // Merge the results.
      diffs = diffs_a;
   ...
   * @return Linked List of Diff objects.
   */
  private LinkedList<Diff<char_t>> diff_lineMode(Text<char_t> text1, Text<char_t> text2,
                                         long deadline) {
    // Scan the text on a line-by-line basis first.
    LinesToCharsResult<char_t> a = diff_linesToChars(text1, text2);
    List<char_t> linearray = a.lineArray;

    LinkedList<Diff<Integer>> atom_diffs =
            new DiffMatchPatch<Integer>().diff_main(a.chars1, a.chars2, false, deadline);
    ...
    diffs.add(new Diff<char_t>(Operation.EQUAL, T.<char_t> empty()));
    int count_delete = 0;
    int count_insert = 0;
    ...
    while (thisDiff != null) {
      ...
      case EQUAL:
        ...
        if (count_delete >= 1 && count_insert >= 1) {
          ...
          for (Diff<char_t> subDiff : diff_main(text_delete, text_insert, false,
              deadline)) {
            pointer.add(subDiff);
          }
        }
   ...
   */
  protected LinkedList<Diff<char_t>> diff_bisect(Text<char_t> text1, Text<char_t> text2,
      long deadline) {
    ...
    for (int d = 0; d < max_d; d++) {
      ...
      for (int k1 = -d + k1start; k1 <= d - k1end; k1 += 2) {
        ...
        while (x1 < text1_length && y1 < text2_length
               && equals(text1.charAt(x1), text2.charAt(y1))) {
          x1++;
          y1++;
        }
        v1[k1_offset] = x1;
      ...
      for (int k2 = -d + k2start; k2 <= d - k2end; k2 += 2) {
        ...
        while (x2 < text1_length && y2 < text2_length
               && equals(text1.charAt(text1_length - x2 - 1),
   ...
   * @return LinkedList of Diff objects.
   */
  private LinkedList<Diff<char_t>> diff_bisectSplit(Text<char_t> text1, Text<char_t> text2,
   ...
   */
  protected LinesToCharsResult<char_t> diff_linesToChars(Text<char_t> text1, Text<char_t> text2) {
    List<char_t> lineArray = new ArrayList<char_t>();
    Map<char_t, Integer> lineHash = new HashMap<char_t, Integer>();
   ...
   */
  private Text<Integer> diff_linesToCharsMunge(Text<char_t> text, List<char_t> lineArray,
    Map<char_t, Integer> lineHash, int maxLines) {
    List<Integer> chars = new ArrayList<Integer>();
    // Walk the text, pulling out a substring for each line.
    // text.split('\n') would would temporarily double our memory footprint.
    // Modifying text would create many large strings to garbage collect.
    for (char_t line: text) {
      if (lineHash.containsKey(line)) {
        chars.add(lineHash.get(line));
      } else {
        lineArray.add(line);
        lineHash.put(line, lineArray.size() - 1);
        chars.add(lineArray.size() - 1);
      }
    }
    return new ArrayText<Integer>(chars.toArray(new Integer[0]));
  }
   ...
   * @param lineArray List of unique strings.
   */
  protected LinkedList<Diff<char_t>> diff_charsToLines(List<Diff<Integer>> diffs,
                                  List<char_t> lineArray) {
   ...
   */
  protected int diff_commonOverlap(Text<char_t> text1, Text<char_t> text2) {
    // Cache the text lengths to prevent multiple calls.
    int text1_length = text1.length();
    int text2_length = text2.length();
    ...
    if (text1_length > text2_length) {
      text1 = text1.substring(text1_length - text2_length, text1.length());
    ...
    int length = 1;
    while (true) {
      Text<char_t> pattern = text1.substring(text_length - length, text1.length());
      int found = text2.indexOf(pattern);
      if (found == -1) {
        return best;
      }
      length += found;
      if (found == 0 || text1.substring(text_length - length, text1.length()).equals(
   ...
   */
  protected HalfMatch<char_t> diff_halfMatch(Text<char_t> text1, Text<char_t> text2) {
    if (Diff_Timeout <= 0) {
    ...
    HalfMatch<char_t> hm1 = diff_halfMatchI(longtext, shorttext,
                                   (longtext.length() + 3) / 4);
    // Check again based on the third quarter.
    HalfMatch<char_t> hm2 = diff_halfMatchI(longtext, shorttext,
                                   (longtext.length() + 1) / 2);
    HalfMatch<char_t> hm;
  ...
  private HalfMatch<char_t> diff_halfMatchI(Text<char_t> longtext, Text<char_t> shorttext, int i) {
    ...
    int j = -1;
    Text<char_t> best_common = T.empty();
    Text<char_t> best_longtext_a = T.empty(), best_longtext_b = T.empty();
    Text<char_t> best_shorttext_a = T.empty(), best_shorttext_b = T.empty();
    while ((j = shorttext.indexOf(seed, j + 1)) != -1) {
      ...
      if (best_common.length() < suffixLength + prefixLength) {
        best_common = shorttext.substring(j - suffixLength, j)
            .concat(shorttext.substring(j, j + prefixLength));
    ...
    if (best_common.length() * 2 >= longtext.length()) {
      return new HalfMatch<char_t>(best_longtext_a, best_longtext_b,
  ...
  public void diff_cleanupSemantic(LinkedList<Diff<char_t>> diffs) {
    ...
    Deque<Diff<char_t>> equalities = new ArrayDeque<Diff<char_t>>();  // Double-ended queue of qualities.
      ...
      if (prevDiff.operation == Operation.DELETE &&
        ...
        if (overlap_length1 >= overlap_length2) {
          if (overlap_length1 >= deletion.length() / 2.0 ||
            ...
            pointer.previous();
            pointer.add(new Diff<char_t>(Operation.EQUAL,
  ...
  public void diff_cleanupSemanticLossless(LinkedList<Diff<char_t>> diffs) {
    ...
    while (nextDiff != null) {
      if (prevDiff.operation == Operation.EQUAL &&
        ...
        bestScore = cleanupSemanticScore.compute(equality1, edit)
            + cleanupSemanticScore.compute(edit, equality2);
        while (edit.length() != 0 && equality2.length() != 0
            && equals(edit.charAt(0), equality2.charAt(0))) {
          equality1 = equality1.concat(edit.charAt(0));
          edit = edit.substring(1).concat(equality2.charAt(0));
          equality2 = equality2.substring(1);
          score = cleanupSemanticScore.compute(equality1, edit)
              + cleanupSemanticScore.compute(edit, equality2);
  ...
  public void diff_cleanupEfficiency(LinkedList<Diff<char_t>> diffs) {
    ...
    boolean changes = false;
    Deque<Diff<char_t>> equalities = new ArrayDeque<Diff<char_t>>();  // Double-ended queue of equalities.
    Text<char_t> lastEquality = null; // Always equal to equalities.peek().text
    ListIterator<Diff<char_t>> pointer = diffs.listIterator();
  ...
  public void diff_cleanupMerge(LinkedList<Diff<char_t>> diffs) {
    diffs.add(new Diff<char_t>(Operation.EQUAL, T.<char_t> empty()));  // Add a dummy entry at the end.
    ListIterator<Diff<char_t>> pointer = diffs.listIterator();
    int count_delete = 0;
    int count_insert = 0;
    ListText<char_t> text_delete = new ListText<char_t>();
      ...
      case EQUAL:
        if (count_delete + count_insert > 1) {
          ...
          if (text_delete.length() != 0) {
            pointer.add(new Diff<char_t>(Operation.DELETE, text_delete));
          }
          if (text_insert.length() != 0) {
            pointer.add(new Diff<char_t>(Operation.INSERT, text_insert));
          }
    ...
    while (nextDiff != null) {
      if (prevDiff.operation == Operation.EQUAL &&
        ...
        if (thisDiff.text.endsWith(prevDiff.text)) {
          ...
          thisDiff.text = thisDiff.text.substring(nextDiff.text.length()).concat(
              nextDiff.text);
          pointer.remove(); // Delete nextDiff.
          nextDiff = pointer.hasNext() ? pointer.next() : null;
          changes = true;
  ...
  public String diff_prettyHtml(List<Diff<char_t>> diffs) {
    ...
    for (Diff<char_t> aDiff : diffs) {
      String text = aDiff.getTextAsString().replace("&", "&amp;").replace("<", "&lt;")
          .replace(">", "&gt;").replace("\n", "&para;<br>");
   ...
   * @return Source text.
   */
  public String diff_text1(List<Diff<char_t>> diffs) {
    StringBuilder text = new StringBuilder();
    for (Diff<char_t> aDiff : diffs) {
      if (aDiff.operation != Operation.INSERT) {
        text.append(aDiff.getTextAsString());
      }
    }
   ...
   */
  public String diff_text2(List<Diff<char_t>> diffs) {
    StringBuilder text = new StringBuilder();
    for (Diff<char_t> aDiff : diffs) {
      if (aDiff.operation != Operation.DELETE) {
        text.append(aDiff.getTextAsString());
  ...
  public String diff_toDelta(List<Diff<char_t>> diffs) {
    ...
    for (Diff<char_t> aDiff : diffs) {
      ...
      case INSERT:
        try {
          text.append("+").append(URLEncoder.encode(aDiff.getTextAsString(), "UTF-8")
                                            .replace('+', ' ')).append("\t");
   ...
   *      boolean values.
   */
  public Object[] patch_apply(LinkedList<Patch<char_t>> patches, String text) {
    if (patches.isEmpty()) {
   ...
   */
  public String patch_addPadding(LinkedList<Patch<char_t>> patches) {
   ...
   */
  public void patch_splitMax(LinkedList<Patch<String>> patches) {
    ...
    boolean empty;
    Operation diff_type;
    List<String> diff_text;
    ListIterator<Patch<String>> pointer = patches.listIterator();
    Patch<String> bigpatch = pointer.hasNext() ? pointer.next() : null;
    while (bigpatch != null) {
      ...
      while (!bigpatch.diffs.isEmpty()) {
        ...
        if (precontext.length() != 0) {
          ...
          patch.diffs.add(new Diff<String>(Operation.EQUAL, precontext));
        }
        while (!bigpatch.diffs.isEmpty()
          ...
          if (diff_type == Operation.INSERT) {
            ...
            empty = false;
            patch.diffs.add(new Diff<String>(diff_type, diff_text));
            bigpatch.diffs.removeFirst();
          } else {
            // Deletion or equality.  Only take as much as we can stomach.
            ListUtils.removeFrom(diff_text, Math.min(diff_text.size(),
            ...
            if (diff_text.equals(bigpatch.diffs.getFirst().text)) {
              ...
              ListUtils.removeFrom(bigpatch.diffs.getFirst().text, diff_text.size());
            }
          }
        }
        ...
        if (postcontext.length() != 0) {
          ...
          if (!patch.diffs.isEmpty()
            ...
            patch.diffs.getLast().text.add(postcontext);
          } else {
            patch.diffs.add(new Diff<String>(Operation.EQUAL, postcontext));
          }
        }
        if (!empty) {
          pointer.add(patch);
   ...
   * @throws IllegalArgumentException If invalid input.
   */
  public List<Patch<String>> patch_fromText(String textline)
      throws IllegalArgumentException {
    List<Patch<String>> patches = new LinkedList<Patch<String>>();
    ...
    while (!text.isEmpty()) {
      ...
      while (!text.isEmpty()) {
        ...
        if (sign == '-') {
          // Deletion.
          patch.diffs.add(new Diff<String>(Operation.DELETE, line));
        } else if (sign == '+') {
          // Insertion.
          patch.diffs.add(new Diff<String>(Operation.INSERT, line));
        } else if (sign == ' ') {
          // Minor equality.
          patch.diffs.add(new Diff<String>(Operation.EQUAL, line));
        } else if (sign == '@') {
          // Start of next patch.
          break;
        } else {
   ...
   */
  public static class Diff<char_t> {
   ...
   */
  public static class Patch<char_t> {
    public LinkedList<Diff<char_t>> diffs;
    public int start1;
    public int start2;
    public int length1;
    public int length2;
     ...
     */
    public Patch() {
      this.diffs = new LinkedList<Diff<char_t>>();
    }
    ...
    public String toString() {
      ...
      for (Diff<char_t> aDiff : this.diffs) {
        switch (aDiff.operation) {
        case INSERT:
          text.append('+');
          break;
        ...
        try {
          text.append(URLEncoder.encode(aDiff.getTextAsString(), "UTF-8").replace('+', ' '))
              .append("\n");
        } catch (UnsupportedEncodingException e) {
          // Not likely on modern system.
          throw new Error("This system does not support UTF-8.", e);
